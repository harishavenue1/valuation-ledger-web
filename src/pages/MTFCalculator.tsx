...
function InstrumentCard({
  ...
  const { totalInv, invested, funded, dayCharge, curPrice } = computeMtfTotals(inst);
    ...
    <div className="p-4 border border-slate-200 rounded-lg mb-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        ...
        <NumberField
          ...
          suffix="%"
          title="Broker's MTF margin requirement for this instrument — the % of Total Inv you must fund yourself"
        />
        <NumberField label="Yearly Interest Rate" value={inst.yearlyRatePct} onChange={(v) => onChange({ ...inst, yearlyRatePct: v })} suffix="%" />
        ...
        <NumberField label="Charges" value={inst.charges} onChange={(v) => onChange({ ...inst, charges: v })} suffix="₹" title="Flat brokerage/charges per round trip, applied in both the leveraged and unleveraged scenarios" />
      </div>
  ...
  return (
    <div>
      ...
      <MethodologyNote>
        ...
        rest, borrowed via MTF at <b>Yearly Interest Rate</b> (charged daily on Funded, compounding ignored — same simple-interest convention
        as the original sheet). <b>Current Price</b> is derived from Buy Price × (1 + Expected P/L%), so the P/L column always ties out exactly
        to (Current − Buy) × Shares. Tax uses STCG below {assumptions.ltcgThresholdDays} days held, LTCG above it (shaded rows), floored at
        zero — a loss doesn't generate a tax credit here. <b>Final Profit</b> = P/L − Tax − Charges − Interest Paid.{" "}
        ...
        to the trade). <b>PAT (No Lev)%</b> = Final Profit (No Lev.) ÷ Invested.
      </MethodologyNote>
